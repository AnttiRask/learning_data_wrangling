...
# boxplot AM vs MPG
mtcars %>% 
  ggplot(aes(as.factor(am), mpg, fill = as.factor(am))) +
  geom_boxplot() +
  labs(
    fill  = "Transmission",
    title = "Boxplot of Miles per Gallon (MPG)",
...
mtcars %>% 
  ...
  labs(
    title = "Effect of Horsepower on Milesper Gallon (MPG)",
    x     = "Horsepower",
    y     = "MPG"
  ) +
  theme_classic()
...
# Line Plot
tibble(x, y, y2, y3) %>%
  pivot_longer(cols = c(y, y2, y3), names_to = "line", values_to = "value") %>%
...
mtcars_counts <- mtcars %>%
  count(cyl, vs, am) %>%
  mutate(across(where(~is_double(.x)), as.factor))
...
You can plot two - or more - graphics side by side. To make that happen, use the function _par()_ and pass a vector with the number of rows and columns you want.

```{r, include=FALSE}
# Plot Graphics

# Create the scatter plot (1,1)
p1 <- mtcars %>% 
  ggplot(aes(disp, hp)) +
...
combined_plot <- p1 + p2 & theme_classic()
```
